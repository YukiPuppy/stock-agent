...
def backtest_trade_plans(
    ...
    for _, plan in plans.iterrows():
        ...
        holding_window = bars.loc[entry_pos : entry_pos + max_holding_days - 1].copy()
...
def evaluate_trade_plan_backtest(backtest_results: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    for keys, group in results.groupby(["strategy_names", "strategy_versions", "action"], dropna=False, sort=True):
        strategy_names, strategy_versions, action = keys
