...
def test_run_trade_plan_backtest_uses_current_signals_when_global_strategy_signals_empty(tmp_path):
    db_path = tmp_path / "stock_agent.duckdb"
    store = StockAgentStore(str(db_path))
    store.save_stock_basic(pd.DataFrame({"code": ["600000"], "name": ["测试股"], "market": ["SH"], "board": ["main"]}))
    store.save_daily_factors(
    ...
    plans, results, performance, diagnostics = run_trade_plan_backtest(
        db_path=str(db_path),
        top_n=1,
        max_plan_items=1,
        strategy_signals=current_signals,
        run_id="run-current-signals",
        return_diagnostics=True,
    )

    assert store.load_strategy_signals().empty
    assert len(plans) == 1
    assert len(results) == 1
    assert len(performance) == 1
    assert diagnostics["historical_signals"] == 1
    assert diagnostics["historical_trade_plans"] == 1
    assert len(store.load_historical_trade_plans(run_id="run-current-signals")) == 1
